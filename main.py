...
def send_email(subject, body, to_email):
    ...
    smtp_port = 587
    smtp_username = 'shreyoshi5000@gmail.com'
    smtp_password = 'twff aazf ldxs zhmc'
...
def convert_to_numeric(value):
    if pd.isna(value) or value is None or (isinstance(value, str) and not value.startswith('₹')):
        return 0
    else:
...
def read_final_result(filename):
    try:
        ...
        rr = ReadResult(filename)
        df = pd.read_csv(filename)
        # print(df.head())
        df[df.columns[3]] = df[df.columns[3]].apply(convert_to_numeric)
        df[df.columns[3]] = df[df.columns[3]].replace('[^0-9]', '', regex=True).astype(int)
        df=df.sort_values(df.columns[3])
        print(df.head())
        cheapest_flight_info = df.iloc[0].values.tolist()
        logger.info(f'Cheapest flight details:{cheapest_flight_info}')
        cheapest_flight_str =' , '.join(map(str, cheapest_flight_info))
        send_email("Cheapest Flight Information", cheapest_flight_str, "ayushutkarsh1012@gmail.com")
